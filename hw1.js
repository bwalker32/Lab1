...
const Product = class Product {
    ...
    static inStock = (products) => {
        const prodArr = products.filter(item => item.availability === "In Stock"); //.map(item => item.name);
        return prodArr;
    }
    ...
    static halfOff = (products) => {
        ...
        products.forEach((item) => {
            let newPrice = item.price * 0.5;
            halfPrice.push(new Product(`${item.name},${newPrice},${item.availability}`));
        });
    ...
    static printProducts = (products) => {
        ...
        products.forEach((item) => {
            console.log(`Product: ${item.name}, Cost: ${item.price}, Availability: ${item.availability}`);
        });
    }
